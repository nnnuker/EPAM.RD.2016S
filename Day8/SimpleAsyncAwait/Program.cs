...
namespace SimpleAsyncAwait
{
    class Program
    {
        ...
        private async static void DoSomeWork()
        {
            ...
            var res = await webClient.DownloadStringTaskAsync(string.Format(searchRequest, "pokemon"));
            var res1 = await webClient.DownloadStringTaskAsync(string.Format(searchRequest, "epam"));
            var res2 = await webClient.DownloadStringTaskAsync(string.Format(searchRequest, "minsk"));
            stopwatch.Stop();
